An external entity in XML is a construct that allows data to be included from an external source within an XML document. External entities are part of the broader XML entity framework, which includes internal entities (defined and used within the same document) and external entities (defined outside the document and referenced from within it).

### Types of External Entities

1. **External General Entities**: These entities reference external files or resources that can be included within the XML document wherever the entity is referenced.
2. **External Parameter Entities**: These entities are used within the Document Type Definition (DTD) and are similar to external general entities but are specifically for use within the DTD.

### Defining and Using External Entities

To define an external entity, you use the `<!ENTITY>` declaration in the DTD section of the XML document.

#### Example: External General Entity

xml

Code kopieren

`<!DOCTYPE example [   <!ENTITY ext SYSTEM "http://www.example.com/external.xml"> ]> <example>   &ext; </example>`

In this example, the external entity `ext` is defined to reference the external resource at `http://www.example.com/external.xml`. The content of this external file will be included at the point where `&ext;` is referenced within the XML document.

#### Example: External Parameter Entity

xml

Code kopieren

`<!DOCTYPE example [   <!ENTITY % ext SYSTEM "http://www.example.com/external.dtd">   %ext; ]> <example>   <!-- content here --> </example>`

In this case, `%ext;` references an external DTD file, and the content of that file is included in the DTD at the point where `%ext;` is used.
