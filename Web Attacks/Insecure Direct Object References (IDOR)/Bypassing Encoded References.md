...
```shell-session
Pangulin@htb[/htb]$ echo -n 1 | md5sum
...
```shell-session
Pangulin@htb[/htb]$ echo -n 1 | base64 -w 0 | md5sum
...
```shell-session
Pangulin@htb[/htb]$ for i in {1..10}; do echo -n $i | base64 -w 0 | md5sum | tr -d ' -'; done
...
```shell-session
Pangulin@htb[/htb]$ bash ./exploit.sh
Pangulin@htb[/htb]$ ls -1
